...
class Engine():
    ...
    def observe(self):
        ...
        voxel = fp_ray.astype(np.int32)
        lt_angles = np.linspace(theta+beta+np.pi/2, 
                                theta+beta-np.pi/2,
                                num=ec.RayNum)
        rt_angles = np.linspace(theta-beta-np.pi/2, 
                                theta-beta+np.pi/2,
                                num=ec.RayNum)

        ray_vec = np.stack((np.cos(lt_angles),
                            np.sin(lt_angles),
                            np.cos(rt_angles),
                            np.sin(rt_angles)),axis=0).astype(np.float32)

        # shape : ((Left/Right), (Cos, Sin), RayNum)
        ray_vec.resize(2,2,ec.RayNum)
        vox_step = ((ray_vec>=0) + (ray_vec<0)*(-1)).astype(np.int32)
        cont_mask = True

        while np.any(cont_mask) :
            target_vox = voxel + vox_step
            cont_mask = np.logical_and.reduce((
                # Hits end of image
                target_vox[:,0,:]>0,
                target_vox[:,1,:]>0,
                target_vox[:,0,:]<self.size[0]-1,
                target_vox[:,1,:]<self.size[1]-1,
                # or anything that is not background color
                np.all(np.logical_or(self._image[target_vox[:,0],
                                   target_vox[:,1]]==colors.COLOR_BACKGROUND,
                                       #TODO : Erase ray
                        self._image[target_vox[:,0],
                                   target_vox[:,1]]==[255,255,0]),
                                   axis=-1)
            ))
            tmax = (target_vox - fp_ray)/ray_vec
            x_mask = (np.abs(tmax[:,0,:]) <=np.abs(tmax[:,1,:]))
            y_mask = (np.abs(tmax[:,0,:]) > np.abs(tmax[:,1,:]))
            voxel[:,0,:] += x_mask*vox_step[:,0,:]*cont_mask
            voxel[:,1,:] += y_mask*vox_step[:,1,:]*cont_mask
            fp_ray[:,1,:] += ray_vec[1]*x_mask*tmax[:,0,:]*cont_mask
            # For accuracy, directly give the int value
            fp_ray[:,0,:] = fp_ray[:,0,:]*np.logical_not(x_mask) + voxel[:,0,:]*x_mask
            fp_ray[:,0,:] += ray_vec[0]*y_mask*tmax[:,1,:]*cont_mask
            # For accuracy, directly give the int value
            fp_ray[:,1,:] = fp_ray[:,1,:]*np.logical_not(y_mask) + voxel[:,1,:]*y_mask
            #TODO: Erase
            self._image[voxel[:,0,:],voxel[:,1,:]] = [255,255,0]
        ...
        lt_obs = self._image[voxel[0,0],voxel[0,1]]
        rt_obs = self._image[voxel[1,0],voxel[1,1]]
        return lt_obs, rt_obs
